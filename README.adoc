...
*   **Unified Command Set:** Provides a consistent API for common package management tasks, regardless of the underlying tool:
    ...
    ** `jpd create` (`c`):** Scaffolds new projects using create runners (supports package names for npm/pnpm/yarn/bun and URLs for deno).
...
*   **External Integrations:** Generate Warp terminal workflows and Carapace completion specs with the `integrate` command. See link:#integrations[Integrations] section.
...
# Via Nix (Linux/macOS)
nix profile install github:nix-community/nur#repos.the-code-fixer-23-nur.jpd
...
[NOTE]
====
jpd create flags and flag passing:

- `--search`, `-s`: Search npm for popular `create-*` packages and pick interactively.
- `--size <n>`: Number of results to show when using `--search` (default: 25).
- Passing flags to scaffolding tools:
  - npm: jpd auto-inserts `--`; do not add your own. If you do, jpd normalizes it.
  - pnpm / yarn v2+ / bun: pass flags directly after your app name.
  - yarn v1: uses npx; pass flags directly.
...
# executes: npm exec create-vite@latest -- my-app --template react
----
====
...
A compact overview of `jpd` commands. For detailed usage, flags, and examples, see the complete https://louiss0.github.io/javascript-package-delegator/reference/commands/[command reference].

.Install and run basics:
* `jpd install` - Install dependencies
* `jpd run dev` - Run script/task

.Package execution:
* `jpd exec create-react-app my-app` - Execute packages
* `jpd dlx create-react-app my-app` - Execute packages with dedicated package runner

.Project scaffolding:
* `jpd create react-app my-app` - Scaffold new projects with package names
* `jpd -a deno create https://deno.land/x/fresh/init.ts my-app` - Scaffold with deno URLs

.Maintenance:
* `jpd update` - Update packages
* `jpd uninstall lodash` - Remove packages  
* `jpd clean-install` - Clean install (CI/CD)

.Agent:
* `jpd agent` - Show detected package manager

.Integrations (see docs for full details):
* `jpd integrate warp --output-dir ./workflows/` - Generate Warp workflow files
* `jpd integrate warp` - Print workflows as multi-doc YAML
* `jpd integrate carapace` - Install Carapace spec globally
* `jpd integrate carapace --stdout` - Print spec to stdout
* `jpd integrate carapace --output ./jpd.yaml` - Write spec to file

[[integrations]]
=== Integrations

Generate external tool integrations with the `integrate` command:

[source,bash]
----
# Warp workflows
jpd integrate warp --output-dir ./workflows/
jpd integrate warp  # print multi-doc YAML to stdout

# Carapace spec
jpd integrate carapace            # install globally (default)
jpd integrate carapace --stdout   # print to stdout
jpd integrate carapace --output ./jpd.yaml
----

For flags, precedence rules, and OS paths, see the https://louiss0.github.io/javascript-package-delegator/reference/commands/#integrate[Integrate command reference].
...
│   ├── create.go       // 'create' command logic (project scaffolding)
│   ├── dlx.go          // 'dlx' command logic (dedicated package runner)
│   ├── exec.go         // 'exec' command logic
│   ├── install.go      // 'install' command logic
│   ├── integrate.go    // 'integrate' command logic (warp, carapace)
...
│   └── root.go
├── internal/integrations // Generators for Warp workflows and Carapace spec
├── jpd-test            // Test utilities and helpers
├── ni                  // (Potentially deprecated or internal alias related to `ni` inspiration)
├── scripts             // Development and CI scripts for testing and cleanup
│   ├── check-coverage.sh // Coverage threshold checking with auto-cleanup
│   └── cleanup.sh       // Clean up test artifacts and accidentally generated binaries
├── LICENSE
