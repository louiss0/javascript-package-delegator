...
*   **Interactive Script/Task Selection (`jpd run` without arguments):** When `jpd run` is executed without arguments, it displays an interactive list of available scripts from `package.json` (or tasks from `deno.json`) for you to choose from.
*   **Interactive Uninstall (`-i`, `--interactive`):** Offers an interactive way to select and uninstall dependencies from your project.
...
*   **Change Directory Flag (`-C`, `--cwd`):** Allows you to execute `jpd` commands in a specified directory without changing your current shell's working directory. This is particularly useful for scripting or managing multiple projects.
...
*   **Shell Auto-completion:** Supports shell auto-completion for the `--agent` flag, providing a convenient list of supported package managers.
...
*   **Run a command in a different directory:**
    [source,bash]
    ----
    jpd install -C /path/to/your/project/
    jpd run dev --cwd ../another-project/
    ----
...
│   └── root.go
├── detect              // Package manager detection logic, now with mockable file system and path lookup
│   └── root.go
