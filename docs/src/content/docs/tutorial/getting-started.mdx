...
<Tabs>
  ...
  <TabItem label="Nix">
    ```bash
    # Install via Nix profile (recommended)
...
<Aside type="note">
  jpd automatically adds the `create-` prefix for npm, pnpm, yarn, and bun. Deno requires a valid URL to a TypeScript initialization script.
</Aside>
