...
```bash
# Print the embedded Carapace completion bridge
jpd _carapace bash
```

The hidden `_carapace` command is intended for Carapace tooling and advanced setups.
For most users, `jpd completion <shell>` remains the primary completion flow.
