...
Generate integration files for external tools like Warp terminal.
...
### Carapace Bridge Command

jpd also exposes a hidden bridge command used by [Carapace](https://carapace.sh/) tooling:

```bash
jpd _carapace bash
```

Use this for advanced Carapace setups; most users should use `jpd completion <shell>`.
